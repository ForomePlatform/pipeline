...
void EM_data::EM_step(const std::vector<std::vector<int> >& AD_list,const std::vector<size_t>& parent_cols,const std::vector<std::vector<double> >& GT_likelihood_wrt_allele_L,const double a,const double b,const std::vector<double>& D){
	...
	for(size_t i(0);i<parent_cols.size();++i){
		const std::vector<int>& AD_temp( AD_list[parent_cols[i]] );
		if(AD_list[parent_cols[i]][0]!=-1 && !(AD_temp[0]==0 && AD_temp[1]==0) ){
...
class each_line_data{
	...
	double AF_unrel_calc(const vcf_line_cols& VLCO,const std::vector<double>& prior_L_new);
};


double each_line_data::AF_unrel_calc(const vcf_line_cols& VLCO,const std::vector<double>& prior_L_new){
	using namespace std;
	vector<double> temp(3);
	double AF_unrel(0);
	double denominator(0);
	const vector<size_t>& parent_cols(VLCO.parent_cols);
	int count(0);
	for(int i(0);i<parent_cols.size();++i){
		const size_t I(parent_cols[i]);
		const vector<int>& AD( AD_list[I] );
		const vector<double>& GL_L( GL_L_list[I] );
		//cout<<count<<")"<<"\t"<<AD<<"\t"<<GL_L<<"\t";
		++count;
		if(AD[0]>=0 && AD[1]>=0 ){
			temp=GL_L+prior_L_new;
			const double max_val( max(temp) );
			temp=temp+(-max_val);
			temp=EXP_1D(temp);
			const double summ( sum(temp) );
			temp=temp*(1.0/summ);
			AF_unrel+=(temp[1]/2.0+temp[2]);
			denominator+=1;
...
double each_line_data::denovo_BGM(const vcf_line_cols& O,const std::vector<double>& prior_L){
	...
	if( M_AD[0]>-1 && D_AD[0]>-1 && C_AD[0]>-1 && M_GL_L[0]<1 && D_GL_L[0]<1 && C_GL_L[0]<1 ){
		...
		P= max_sum_data_obj.GT_posterior_probty[4][2] ;
	}
	return P;
